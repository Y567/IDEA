...
public class PoetryDaoImpl implements PoetryDao {
    ...
    public List<Echart> findByAmount(int left, int right) {
        ...
        try {
            ...
            //2.定义sql语句
            String sql = "select author,count(*) as total from tangshi group by author having total >= ? and total <=? order by total DESC";
            //3.创建执行对象
    ...
    public Map<String, Integer> findWordsByName(String name) {
        ...
        try {
            ...
            if (name == null||"".equals(name)) {
                ...
                //查询指定的诗人
                sql = "select words from tangshi group by having author = ?";
                pstm = con.prepareStatement(sql);
    ...
    public List<String> findPoetByName(int i) {
        ...
        try {
            ...
            String sql;
            //3.创建执行对像
            sql = "SELECT DISTINCT author FROM tangshi GROUP BY author HAVING author LIKE ?";
            pstm = con.prepareStatement(sql);
            if(i == 2){
                pstm.setString(1,"__");
            }else{
